...
def _justify(sc: int, regime_label: str, score: Any, contracts: int, hurst: Any) -> str:
    ...
    if sc == 2:
        return (
            ...
            f"{contracts} MNQ. Fall back to LiqSweep if ORB fails to extend."
        )
    if sc == 3:
        return (
            f"Regime {regime_label}{score_part}{hurst_part}. LiqSweep iFVG — "
            f"sweep + discount/premium DOL entries, {contracts} MNQ. "
            f"Best on ranging days with clear Asia/London liquidity pools."
        )
    if sc == 4:
        return (
